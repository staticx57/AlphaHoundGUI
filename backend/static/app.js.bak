...
function renderDashboard(data) {
    ...
    // Chart
    // [STABILITY] Check visibility before rendering to save resources
    if (!isPageVisible) {
        console.log('Page hidden, skipping chart render');
        return;
    }
...
handleFile = async function (file) {
    // [STABILITY] Check if user is currently acquiring data to prevent accidental overwrite
    if (isAcquiring) {
        if (!confirm('Recording in progress. Stop recording and load this file?')) {
            return;
        }
        stopAcquisition();
    }
...
// [STABILITY] Prevent accidental tab closure if acquiring
...
// [STABILITY] Visibility API - Pause rendering when tab hidden
...
document.addEventListener('visibilitychange', () => {
    ...
    if (isPageVisible) {
        ...
        if (currentData) {
            ...
            // Check chart type
            const scaleType = (chart && chart.options.scales.y.type) ? chart.options.scales.y.type : 'linear';
            if (compareMode && overlaySpectra.length > 0) {
                renderComparisonChart(scaleType);
            } else {
                renderChart(currentData.energies, currentData.counts, currentData.peaks, scaleType);
            }
        }
    }
});
...
document.getElementById('btn-add-file').addEventListener('click', () => {
    // [STABILITY] Cap overlays to prevent memory issues
    if (overlaySpectra.length >= 8) {
        alert('Maximum of 8 spectra allowed for comparison to ensure performance.');
        return;
    }
...
// WebSocket dose monitoring with [STABILITY] Auto-Reconnect
...
function setupDoseWebSocket() {
    if (doseWebSocket) return; // Already connected or connecting

    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    const wsUrl = `${protocol}//${window.location.host}/ws/dose`;

    // Reset display
    document.getElementById('dose-display').textContent = 'Connecting...';

    doseWebSocket = new WebSocket(wsUrl);

    doseWebSocket.onopen = () => {
    ...
    doseWebSocket.onclose = () => {
        ...
        doseWebSocket = null;
        document.getElementById('dose-display').textContent = 'Disconnected';

        // Attempt reconnect if not explicitly stopped
        // (We don't have an explicit 'isStopped' flag for WS, but we can assume we always want it 
        // unless the user clicked disconnect device. Let's check device status logic)
        // Ideally, we only reconnect if the "Device" is supposed to be connected.
        // But for this simple app, we can just try to reconnect up to a point or forever.
...
function attemptReconnect() {
    ...
    reconnectTimer = setTimeout(() => {
        ...
        // We will try. backend checkDeviceStatus will confirm if real hardware is there.
        setupDoseWebSocket();
    }, delay);
}
...
function stopDoseMonitoring() {
    ...
    if (doseWebSocket) {
        // Remove onclose handler to prevent reconnect loop when manually stopping
        doseWebSocket.onclose = null;
        doseWebSocket.close();
        doseWebSocket = null;
    }
    document.getElementById('dose-display').textContent = '-- µRem/hr';
